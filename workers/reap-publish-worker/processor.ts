...
import { publishClip, getIntegrations, ReapApiError } from "@/lib/reap";
...
export async function processReapPublishJob(job: Job<ClipUploadJobData>) {
  ...
  try {
    ...
    await logger.info("Reap publish request accepted. Post is now processing.", {
      phase: 5,
      clipId,
      uploadTargetId,
      reapPostId: post.id,
    ...
    if (post.status === "completed" && post.successPlatforms.includes("tiktok")) {
      await markUploadCompleted(uploadTargetId, clip, post);
    } else if (post.status === "failed" || (post.failedPlatforms?.includes("tiktok") && !post.successPlatforms.includes("tiktok"))) {
